...
class RMSprop(object):
    ...
    def __init__(self, learning_rate, beta=0.9, lr_decay=None):
        self.params = None
        self.grads = None
        self.v = {}
        self.lr = learning_rate
    ...
    def step(self, epoch):
        ...
        if self.lr_decay_rate:
            for i in self.lr_decay_epoches:
                lr *= self.lr_decay_rate ** (epoch >= i)
        for key, param in self.params.items():
            self.v[key] = self.beta * self.v[key] + (1 - self.beta) * np.square(self.grads[key][0])
            param[0] -= (lr / (np.sqrt(self.v[key] + 1e-8))) * self.grads[key][0]
